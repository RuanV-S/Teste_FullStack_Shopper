...
export default function Page() {
  ...
  const handleConfirmRide = async () => {
    ...
    if (response.status === 200) {
      const responseDrivers = await fetch("http://localhost:8080/drivers");
  ...
  const onSubmit: SubmitHandler<FormData> = async ({
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        const responseMap = await fetch("http://localhost:8080/map-static", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
  ...
  if (showHistory) {
    const onSubmitFilter: SubmitHandler<FormData> = async (data) => {
      try {
        let url = `http://localhost:8080/ride/${data.customer_id}`;
